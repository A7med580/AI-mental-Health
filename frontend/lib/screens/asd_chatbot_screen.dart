...
class AsdChatbotScreen extends StatefulWidget {
  final Map<String, dynamic>? questionnaireAnswers;

  const AsdChatbotScreen({
    Key? key,
    this.questionnaireAnswers,
  }) : super(key: key);

  @override
  State<AsdChatbotScreen> createState() => _AsdChatbotScreenState();
}

class _AsdChatbotScreenState extends State<AsdChatbotScreen> {
  ...
  final String _initialQuestion = "How do you typically feel in busy or crowded social situations?";
  ...
  Future<void> _handleSend() async {
    ...
    try {
      final followUp = await GeminiService().generateChatbotQuestion(text, 'autism', _questionCount);
      if (mounted) {
        setState(() => _isLoading = false);
        if (followUp != null && followUp.isNotEmpty) {
          _addAssistantMessage(followUp);
        } else {
          _addAssistantMessage("I understand. Do you find comfort in specific routines?");
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Icon(asdTheme.icon, size: 18, color: asdTheme.accentColor),
            const SizedBox(width: 8),
            Text('ASD Chat', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          ],
        ),
      ),
      body: SafeArea(
                    ...
                    child: Container(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: isUser ? asdTheme.accentColor.withValues(alpha: 0.9) : AppColors.cardWhite,
                  ...
                  ? SizedBox(
                      ...
                      child: ElevatedButton(
                        style: ElevatedButton.styleFrom(
                          backgroundColor: asdTheme.accentColor,
